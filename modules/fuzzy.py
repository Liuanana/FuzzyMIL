...
if __name__ == "__main__":
    ...
    # rrt+abmil
    fuzzy_mil = FuzzyMIL(n_classes=2,epeg_k=15,crmsa_k=3)
    x = fuzzy_mil(x)  # 1,N,D -> 1,C

    # rrt. you should put the rrt_enc before aggregation module, after fc and dp
    # x_rrt = fc(x_rrt) # 1,N,1024 -> 1,N,512
    # x_rrt = dropout(x_rrt)
    fuzzy = GFEEncoder(mlp_dim=128,epeg_k=15,crmsa_k=3)
    x_fuzzy = fuzzy(x_fuzzy) # 1,N,512 -> 1,N,512
