...
import argparse, os
from modules import attmil,clam,dsmil,transmil,mean_max,fuzzy,attmil_ibmil
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    # bulid networks
    if args.only_rrt_enc and args.model != 'fuzzymil':
        model_params = {
        ...
        rrt_enc = fuzzy.GFEEncoder(**model_params).to(device)
    else:
        rrt_enc = None

    if args.model == 'fuzzymil':
        model_params = {
        ...
        #model = fuzzy.RRTMIL(**model_params).to(device)
        model = fuzzy.FuzzyMIL(**model_params).to(device)
